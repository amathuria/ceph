  ...
  bool with_atomic_roll_segment)
{
  ...
  return std::move(ret.future
    ...
    t.get_phase_durations().ool_seg_io +=
  ...
  std::list<CachedExtentRef>& extents)
{
  ...
  if (!record_submitter.is_available()) {
    ...
    return trans_intr::make_interruptible(
      ...
      t.get_phase_durations().ool_seg_wait +=
  ...
  for (auto it = extents.begin(); it != extents.end();) {
    ...
    if (action == action_t::ROLL) {
      ...
      const auto roll_start = std::chrono::steady_clock::now();
      return trans_intr::make_interruptible(
        record_submitter.roll_segment(
        ).safe_then([fut_write=std::move(fut_write), &t, roll_start]() mutable {
          t.get_phase_durations().ool_seg_roll +=
            std::chrono::steady_clock::now() - roll_start;
  ...
  std::list<CachedExtentRef>& extents)
{
  ...
  return trans_intr::make_interruptible(
    ...
    t.get_phase_durations().ool_rbm_io +=
      std::chrono::steady_clock::now() - io_start;
  });
}
