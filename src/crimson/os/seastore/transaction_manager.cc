  ...
  std::optional<journal_seq_t> trim_alloc_to)
{
  ...
  co_await epm->write_preallocated_ool_extents(tref, allocated_extents);
  {
    const auto elapsed = std::chrono::steady_clock::now() - ool_start;
    tref.get_phase_durations().ool_write += elapsed;
    tref.get_phase_durations().ool_rbm += elapsed;
  }
