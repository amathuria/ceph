  ...
  return enter_stage<>(
	...
	return shard_services.get_map(next_epoch).then(
	  [this, FNAME] (cached_map_t&& next_map) {
        ...
        // Use the last OSDMap epoch to check for splits consecutively.
        epoch_t last_epoch = pg->get_osdmap_epoch();
        pg->handle_advance_map(next_map, rctx);
        DEBUG("{}: checking for splits between {} and {}",
              *this, last_epoch, next_map->get_epoch());
        return check_for_splits(last_epoch, next_map);
	  });
      }).then([this, FNAME] {
	pg->handle_activate_map(rctx);
