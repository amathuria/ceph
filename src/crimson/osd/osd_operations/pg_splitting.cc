...
PGSplitting::PGSplitting(
  Ref<PG> pg, ShardServices &shard_services, epoch_t next_epoch,
  PeeringCtx &&rctx)
  : pg(pg), shard_services(shard_services), next_epoch(next_epoch),
    rctx(std::move(rctx))
{
...
seastar::future<> PGSplitting::start()
{
  ...
  if (last_map->have_pg_pool(pg->get_pgid().pool())) {
    ...
    logger().debug("{} pool doesn't exist in {}", pg->get_pgid(), last_map);
    return seastar::now();
  }
  ...
  return shard_services.get_map(next_epoch).then(
	 ...
	 return seastar::do_with(pg, children, 
				 ...
				 split_pgs, [this] (auto &pg,
						    ...
						    auto &new_pg_num,
						    auto &split_pgs) {
	  return seastar::do_for_each(children, [this, &pg, &new_map, &new_pg_num, &split_pgs] (auto& child_pg) {
	    return shard_services.get_or_create_pg(child_pg).then([this, child_pg]
		 (auto core) {
		 logger().debug(" PG {} mapped to {}", child_pg.pgid, core);
		 return seastar::now();
	       }).then([this, child_pg, new_map=new_map] {
		 logger().debug(" {} map epoch: {}", child_pg.pgid, new_map->get_epoch());
		 return shard_services.make_pg(std::move(new_map), child_pg, true);
	 ...
	 }).then([this, &children, &new_map, &pg, &split_pgs] {
	   split_stats(split_pgs, children);
	   return seastar::do_for_each(split_pgs, [this, &new_map] (auto& child_pg) {
	       logger().debug(" advance map for {}", shard_services.get_map()->get_epoch());
